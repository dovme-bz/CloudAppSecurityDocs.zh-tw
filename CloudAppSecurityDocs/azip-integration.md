...
此功能目前適用於儲存於 Box、G Suite、SharePoint Online 和商務用 OneDrive 中的檔案。 未來版本中將支援更多雲端應用程式。
...
> [!NOTE]
> 透過檔案原則以自動套用 Azure 資訊保護標籤，是一種強大的功能。 為了避免客戶錯誤地將標籤套用至大量檔案，因此會有每個租用戶每個應用程式每日限制 100 個**套用標籤**動作的安全措施。 達到每日限制之後，套用標籤動作便會暫時暫停，並會在次日 (12:00 UTC 之後) 自動繼續。 若要提高租用戶的限制，請[連絡 Cloud App Security 支援](mailto:cascoresupport@microsoft.com)。
