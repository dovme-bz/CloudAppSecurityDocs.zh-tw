...
在組織受到 Cloud App Security 保護之後，所有雲端活動都會根據各種預先定義的風險因素進行評分。 Cloud App Security 會查看雲端上每位使用者的工作階段，然後考量您在此處設定的風險因素以警示您有不同於組織基準的事故發生，或不同於使用者一般活動的事故發生。 原則可以針對不同的使用者、位置和組織單位以不同的方式來執行。 例如，您可以建立一項原則，在您的 IT 小組成員從辦公室外部進行活動時發出警示。  

Cloud App Security 有七天的初始學習期間，在此期間，不會將任何新的使用者、活動、裝置或位置標記為異常。 之後，每個工作階段都會與活動進行比較，當使用者處於使用中時，會偵測到上個月的 IP 位址、裝置等以及這些活動的風險分數。 


可用的異常偵測原則如下：

**不可能的移動**
- 這項偵測是啟發式異常偵測引擎的一部分，會依據從您組織活動中學習到的基準，分析您的環境及觸發警示。 其發現到有兩個使用者活動 (可能是單一或多個工作階段) 來自相距遙遠的地理位置，但使用者從第一個位置移動到第二個位置的時間短於應當花費的時間，就表示有不同使用者在使用同一個認證。 這項偵測利用機器學習演算法略過導致不可能移動條件的明顯「誤判」，例如組織中其他使用者定期使用的 VPN 和位置。 該偵測一開始會先學習 7 天，並在這段期間學習新使用者的活動模式。


**非經常性國家/地區的活動**
- 這項偵測是啟發式異常偵測引擎的一部分，會依據從您組織活動中學習到的基準，分析您的環境及觸發警示。 這項偵測會考量過去的活動位置來判斷新的和非經常性的位置。 異常偵測引擎會儲存組織中使用者先前使用的位置資訊。 若活動發生自使用者或組織中任何使用者最近未前往或從未前往過的位置，則會觸發警示。 該偵測一開始會先學習 7 天，在這段期間不會對任何新位置發出警示。


**匿名 IP 位址的活動**
- 這項偵測是啟發式異常偵測引擎的一部分，會依據從您組織活動中學習到的基準，分析您的環境及觸發警示。 這項偵測會識別使用者透過已識別為匿名 Proxy IP 位址的 IP 位址來活動。 想隱藏裝置 IP 位址的人會使用這類 Proxy，並可能用於惡意用途。 這項偵測利用機器學習演算法減少「誤判」，例如組織中使用者普遍使用的誤標 IP 位址。

**可疑 IP 位址的活動**
- 這項偵測是啟發式異常偵測引擎的一部分，會依據從您組織活動中學習到的基準，分析您的環境及觸發警示。 這項偵測會識別使用者透過 Microsoft 威脅情報已識別為具風險的 IP 位址來活動。 這些 IP 涉及惡意活動，例如殭屍網路 C&C，且可能表示帳戶已洩露。 這項偵測利用機器學習演算法減少「誤判」，例如組織中使用者普遍使用的誤標 IP 位址。


**X 異常活動 (依使用者)**
- 這項偵測是啟發式異常偵測引擎的一部分，會依據從您組織活動中學習到的基準，分析您的環境及觸發警示。 這項偵測依據學習到的基準，識別在單一工作階段中執行多個 X 活動的使用者，這可能表示有缺口嘗試。 這項偵測利用機器學習演算法分析使用者登入模式，並減少「誤判」。


**多次失敗的登入嘗試**
- 這項偵測是啟發式異常偵測引擎的一部分，會依據從您組織活動中學習到的基準，分析您的環境及觸發警示。 此偵測依據學習到的基準，識別在單一工作階段中多次嘗試登入失敗的使用者，這可能表示缺口嘗試。 這項偵測利用機器學習演算法分析使用者登入模式，並減少「誤判」。
...
若要控制原則觸發的警示數量，請設定 [每日警示限制] 限制一天中產生的警示數量。  
