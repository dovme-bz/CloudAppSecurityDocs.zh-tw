...
3. 此時，您可以前往 [調查] > [檔案]，然後按一下檔案，以檢視內容檢查的狀態。 在隨即開啟且內含檔案詳細資料的檔案抽屜中，[內容檢查狀態] 會顯示 [已完成]、[待決]、[不適用] (若檔案類型不受支援) 或失敗訊息。 如需內容掃描失敗訊息的資訊，請參閱[為內容檢查進行疑難排解](troubleshooting-content-inspection.md)。
